...
interface SuratTugasDetailModalProps {
  item: SuratTugasItem | null;
  onClose: () => void;
}

export const SuratTugasDetailModal: React.FC<SuratTugasDetailModalProps> = ({
  item,
  onClose
}) => {
  if (!item) return null;
    ...
    <Modal isOpen={!!item} onClose={onClose} title={modalTitle} maxWidth="xl">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 rounded-xl bg-slate-950/80 border border-slate-800 text-xs">
          <div>
            <span className="text-slate-400 block mb-1">Nomor S:</span>
            {item.noS ? (
          ...
          <div>
            <span className="text-slate-400 block mb-1">Nomor ST:</span>
            {item.noST ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800">
            <span className="text-slate-400 block mb-1">Tanggal Surat Tugas:</span>
          ...
          <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800">
            ...
            <div className="flex items-center gap-1.5 text-emerald-400 font-medium">
              ...
              <span>{item.suratDiterimaSekretaris || '-'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950/80 border border-slate-800 text-xs">
          <span className="text-slate-400 block mb-1">Waktu Pelaksanaan Tugas:</span>
